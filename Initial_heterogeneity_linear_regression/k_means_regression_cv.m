function [num_partitions_best,combine_principle_best,split_principle_best,random_number_best,cv_matrix] = k_means_regression_cv(num_partitions_list,eps_initial,eps_out,iter_max_initial_in,...
        ...
        for jj = 1:length(combine_principle_list)
            for qq = 1:length(split_principle_list)
                ...
                try
                    ...
                    cv_matrix(ii,jj,qq,kk) = log(cv_matrix(ii,jj,qq,kk)) + 5*((log(sample_size)/sample_size)*(col_size+row_size)*length(partitions));
                catch
