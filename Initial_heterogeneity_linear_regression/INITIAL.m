function [Result_table, Result_table_summary] = INITIAL(simulation_size, Data_x, y_real, sample_size, row_size, col_size, beta_real, gamma_real,...
                                                        num_partitions_list, eps_initial, eps_out, iter_max_initial_in, iter_max_initial_out,...
                                                        combine_principle_list, split_principle_list, random_number_list, disp_if)
%-------------------------------------函数功能----------------------------------------------------%
% 使用初值搜索算法的异质矩阵回归模型，返回聚类数，sc值，均方误差等参数以及估计的初始值(可多次模拟)
%-----------------------------------------使用方式--------------------------------------------%
% [Result_table, Result_table_summary] = INITIAL(simulation_size, Data_x, y_real, sample_size, ...
...
parfor i = 1:simulation_size
    y = y_real(:, i);
    x = Data_x(:,:,:,i);
    [~, ~, partitions, result] = k_means_regression_initial(num_partitions_list, eps_initial, eps_out,...
...
save('D:\MATLAB_Document\ADMM_BCD_new\多方法结果\初值\非平衡设计\mu=1\B2\Result_table',"Result_table")
save('D:\MATLAB_Document\ADMM_BCD_new\多方法结果\初值\非平衡设计\mu=1\B2\Result_table_summary',"Result_table_summary")
end