function [beta0, gamma0, partitions, result] = k_means_regression_initial(num_partitions_list, eps_initial, eps_out,...
...
%k_means_regression_initial(num_partitions_list, eps_initial, eps_out,...
%                                                    iter_max_initial_in, iter_max_initial_out,...
%                                                   combine_principle_list, split_principle_list, random_number_list,...
%                                                    sample_size, row_size, col_size, x, y, disp_if,...
%                                                    beta_real, gamma_real)
...
if isempty(iter_max_initial_in)
    iter_max_initial_in = 10;
end
if isempty(iter_max_initial_out)
    iter_max_initial_out = 10;
end
...
if isempty(split_principle_list)
    split_principle_list = 0.6*sample_size;
end
...
[num_partitions_best,combine_principle_best,split_principle_best,random_number_best, cv_matrix] = k_means_regression_cv(num_partitions_list,eps_initial,eps_out,iter_max_initial_in,...
...
[beta0,gamma0,coefficients,partitions,combine,residual_coefficients] = k_means_regression(num_partitions_best,eps_initial,eps_out,iter_max_initial_in,...
    iter_max_initial_out,combine_principle_best,split_principle_best,sample_size,row_size,col_size,x,y,random_number_best);
