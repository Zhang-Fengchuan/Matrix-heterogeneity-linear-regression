function [beta_back,gamma_back,class_back,index_back,subgroup_number,...
    per_if,mse_beta,mse_gamma,sc,BIC_back,BIC_part2_back,wucha_back,convergence,iter_back,Data] = admm4(sample_size,row_size,col_size,iter_max1,iter_max2,iter_max3,eps1,eps2,eps3,...
...
%----------------------------------------ADMM第一步-------------------------------------------%
%---------------------------------------------------------------------------------------------%
    [beta,gamma,~] = admm3(sample_size,row_size,col_size,iter_max1,iter_max2,eps1,eps2,beta,gamma,H_p,H_q,X,y,k,v,...
...
%--------------------------------------------系数差和原始残差计算-----------------------------------------------%
    if full_if == 1
        ...
        residual_vec = zeros(combine_number*(row_size+col_size),1);
        for i = 1:combine_number
            residual_vec(((i-1)*row_size+1):(i*row_size),1) = kron(E(i,:),eye(row_size))*beta;
            residual_vec(((combine_number*row_size)+((i-1)*col_size+1)):((combine_number*row_size)+(i*col_size)),1) = kron(E(i,:),eye(col_size))*gamma;
        end
        residual_primal = norm((residual_vec-[v_vec;w_vec])/sqrt(0.5*sample_size*(sample_size-1)*(row_size+col_size)));
    else
...
%-----------------------------------------------计算预测误差值---------------------------------------------------%
wucha = log(sum((y-kron(eye(sample_size),ones(row_size,1)')*diag(beta)*X*gamma).^2)/sample_size);
%------------------------------------------------计算BIC值-------------------------------------------------------%
