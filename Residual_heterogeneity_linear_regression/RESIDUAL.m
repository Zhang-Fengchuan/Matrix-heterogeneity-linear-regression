function [Result_table, Result_table_summary] = RESIDUAL(simulation_size, Data_x, y_real, sample_size, row_size, col_size, beta_real, gamma_real,...
                         eps_initial, iter_max_initial_in, k_min, k_max, num_time)
%-------------------------------------函数功能----------------------------------------%
...
for i = 1:simulation_size
    y = y_real(:, i);
    x = Data_x(:,:,:,i);
    [result] = residual_single_regression(x, y, sample_size, row_size, col_size, beta_real, gamma_real,...
                                            eps_initial, iter_max_initial_in, k_min, k_max, num_time);
    ...
    fprintf("已完成基于同质回归残差聚类的异质矩阵回归模型第%.1f %%\n",(i/simulation_size)*100)
end
...
save('D:\MATLAB_Document2\ADMM_BCD_new\多方法结果\RESIDUAL\非平衡设计\mu=1\B2\Result_table',"Result_table")
save('D:\MATLAB_Document2\ADMM_BCD_new\多方法结果\RESIDUAL\非平衡设计\mu=1\B2\Result_table_summary',"Result_table_summary")
end
